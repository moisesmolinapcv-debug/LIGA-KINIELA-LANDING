...
export const BottomNavBar: React.FC<BottomNavBarProps> = ({ prizeText = 'Premio' }) => {
  ...
  return (
    <div className="fixed bottom-2 sm:bottom-3 left-0 right-0 z-50 px-2 sm:px-6 pointer-events-none lg:hidden animate-fadeIn">
      <div className="max-w-md mx-auto relative pointer-events-auto">
        {/* Contenedor Flotante Tipo Vidrio Templado Translúcido */}
        <nav
          aria-label="Navegación rápida móvil"
          className="relative rounded-2xl sm:rounded-3xl bg-[#00063E]/90 backdrop-blur-xl border border-[#FFAF3F]/30 shadow-[0_8px_32px_rgba(0,4,40,0.65)] px-1 sm:px-2 py-1 sm:py-1.5 flex items-center justify-between"
        >
          {/* 1. Atajo: Inicio */}
          <button
            type="button"
            onClick={() => scrollTo('top', 'inicio')}
            className={`flex-1 flex flex-col items-center justify-center py-1 sm:py-1.5 px-0.5 sm:px-1 rounded-xl transition-all duration-300 relative ${
          ...
          >
            {activeSection === 'inicio' && (
              <span className="absolute inset-0 bg-[#FFAF3F]/15 rounded-xl blur-sm" />
            )}
            <Home
            ...
            />
            <span className="text-[9px] sm:text-[10px] font-bold mt-0.5 tracking-tight">Inicio</span>
            {activeSection === 'inicio' && (
          ...
          >
            ...
            />
            <span className="text-[9px] sm:text-[10px] font-bold mt-0.5 tracking-tight">Partidos</span>
            {activeSection === 'partidos' && (
              <span className="w-1.5 h-1.5 rounded-full bg-[#FFAF3F] mt-0.5 animate-pulse" />
            )}
          </button>
          ...
          <div className="flex-1 flex flex-col items-center justify-center -mt-3.5 sm:-mt-5">
            <button
              type="button"
              onClick={() => scrollTo('premios', 'premios')}
              aria-label="Ver Pozo de Premios"
              className={`relative w-11 h-11 sm:w-13 sm:h-13 rounded-full flex items-center justify-center transition-all duration-300 shadow-xl active:scale-95 group ${
                isCenterActive
                  ? 'bg-gradient-to-tr from-[#882445] via-[#a32b53] to-[#FFAF3F] border-2 border-[#FFAF3F] shadow-[0_0_20px_rgba(255,175,63,0.6)] scale-105'
                  : 'bg-gradient-to-tr from-[#060e4f] to-[#0c186b] border-2 border-[#FFAF3F]/50 shadow-[0_0_12px_rgba(0,6,62,0.8)]'
              }`}
            >
              {/* Resplandor pulsante */}
              <span
            ...
            <span
              className={`text-[9px] sm:text-[10px] font-black uppercase tracking-wider mt-0.5 transition-colors ${
          ...
          >
            ...
            />
            <span className="text-[9px] sm:text-[10px] font-bold mt-0.5 tracking-tight">Campeones</span>
            {activeSection === 'campeones' && (
              <span className="w-1.5 h-1.5 rounded-full bg-[#FFAF3F] mt-0.5 animate-pulse" />
            )}
          </button>
          ...
          >
            ...
            />
            <span className="text-[9px] sm:text-[10px] font-bold mt-0.5 tracking-tight truncate max-w-[50px] sm:max-w-none">Aliados</span>
            {activeSection === 'aliados' && (
              <span className="w-1.5 h-1.5 rounded-full bg-[#FFAF3F] mt-0.5 animate-pulse" />
            )}
          </button>
