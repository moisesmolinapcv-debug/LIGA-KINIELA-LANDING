...
import { ShieldCheck, Trophy, Calendar, HelpCircle, BookOpen, Menu, X, Lock } from 'lucide-react';
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-kiniela-navy/95 backdrop-blur-md py-2.5 border-b border-kiniela-gold/20 shadow-lg shadow-kiniela-navy-deep/70'
          : 'bg-gradient-to-b from-kiniela-navy-deep/90 via-kiniela-navy/60 to-transparent py-4'
      }`}
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group focus:outline-none">
            ...
            <div className="flex flex-col">
              <span className="font-extrabold text-lg sm:text-xl tracking-wider text-white uppercase leading-none font-sans">
                LIGA <span className="text-kiniela-gold">KINIELA</span>
              </span>
              <span className="text-[10px] sm:text-xs text-kiniela-gold-light tracking-widest font-medium uppercase mt-0.5">
                Plataforma Oficial
              </span>
            </div>
          </Link>
          ...
          {/* Trust Badges & Discreet Admin Link */}
          <div className="hidden sm:flex items-center gap-3">
            ...
            <Link
              ...
              className="p-2 rounded-lg text-slate-400 hover:text-kiniela-gold hover:bg-white/5 transition-colors"
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <Link
              href="/admin"
              className="p-2 text-slate-400 hover:text-kiniela-gold"
        ...
        <div className="lg:hidden bg-kiniela-navy-deep/98 border-b border-kiniela-gold/20 backdrop-blur-xl px-4 pt-3 pb-6 space-y-2 animate-fadeIn">
          ...
          <div className="pt-3 border-t border-white/10 flex items-center justify-between text-xs text-slate-400 px-2">
            ...
            <Link
              href="/admin"
              onClick={() => setMobileMenuOpen(false)}
              className="text-kiniela-gold-light hover:underline flex items-center gap-1"
            >
              <Lock className="w-3.5 h-3.5" /> Acceso Admin
            </Link>
          </div>
        </div>
      )}
